...
points:
```math
X_l =  \{X_l^{[0]},X_l^{[1]}, ... , X_l^{[m]}\}, X_l^{[j]} \in \mathbb{R}^3.
```
The perturbation of the robot position is a vector in $\mathbb{R}^3$:
```math
\Delta x_r^{[i]} =
\begin{bmatrix}\delta x \\ \delta y \\ \delta \theta
\end{bmatrix}
```
The perturbation of the landmark position is a vector in $\mathbb{R}^3$:
```math
\Delta x_l^{[j]} =
...
- Pose-Pose
```math
h(X_r^{i}\boxplus\Delta x_r^{[i]},X_r^{j}\boxplus\Delta x_r^{[j]})
```
computed as the pose measurements above
- Pose-Projection:
```math
h(X_r\boxplus \Delta x_r, X_l + \Delta x_l) =
\pi(K * {}^cX_r(X_r\boxplus \Delta x_r)^{-1}(X_l + \Delta x_l))
```
with $\pi$ being the projection function, $K$ the intrinsics matrix of the camera,
...
This leads to an improved initial guess of the landmark position, which results
